...
Version 2.1 keeps V1 unchanged and provides an independent lessor lifecycle scenario builder. One scenario can start at an arbitrary analysis date and contain a current lease plus any number of consecutive future leases. Physical component state continues across leases while contract reserve accounts close separately. The primary output covers maintenance-reserve collections, event cost, reserve reimbursement, lessee-unfunded exposure and lease-end reserve close-out. Rent, transition-period assumptions and whole-aircraft investment returns are outside the active dashboard; multi-scenario comparison is optional and is not based on a mandatory NPV ranking.
